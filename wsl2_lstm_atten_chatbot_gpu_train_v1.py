...
def preprocess(inp, targ):
    ...
    inp.set_shape([None])  # Enforce rank 1
    targ.set_shape([None])  # Enforce rank 1
    return inp, targ


def filter_max_length(inp, targ):
    # Ensure non-empty sequences and within max length
...
train_dataset = (
    ...
    .cache('/tmp/train_cache')  # Cache the dataset
...
def train_step(input_batch, target_batch):
    ...
    with tf.GradientTape() as tape:
        ...
        for t in range(1, target_batch.shape[1]):
            ...
            # Debugging: Check input, output, and prediction tensors
            tf.print("Input:", input_batch[0])
            tf.print("Target:", target_batch[0])
            tf.print("Predictions:", tf.argmax(predictions[0], axis=-1))
...
# Optional debug for dataset
example_pair = next(iter(train_data))
tf.print("Example Input Sentence:", example_pair[0])
tf.print("Example Target Sentence:", example_pair[1])
...
for epoch in range(EPOCHS):
    ...
    for batch, (input_batch, target_batch) in enumerate(train_dataset.take(num_batches)):
        ...
        if batch < 5:  # Debugging only the first 5 batches
            tf.print("Input:", input_batch[0])
            tf.print("Target:", target_batch[0])
            tf.print("Batch Loss:", batch_loss)
        if (batch + 1) % 10 == 0:
            print(f" Batch {batch + 1} Loss: {batch_loss.numpy():.4f}")
    avg_loss = total_loss / num_batches
    print(f"Epoch {epoch + 1} Loss: {avg_loss:.4f}")

encoder.build(input_shape=(None, MAX_LENGTH))  # Build encoder
decoder.build(input_shape=[(None, 1), (None, MAX_LENGTH, LSTM_UNITS), (None, LSTM_UNITS), (None, LSTM_UNITS)])  # Build decoder
